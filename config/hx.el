...
(defun hx-toggle-multiple-cursors ()
  ...
  (if (not multiple-cursors-mode)
    ...
    ;; remove highlighted texts first
    (call-interactively #'hx-no-sel)
...
(modaled-define-keys
  ...
  `(("j" . ("left" . ,(hx :re-sel :eval backward-char)))
    ...
    ("C-l" . ("up (text)" . ,(hx :let (line-move-visual nil) :re-sel :eval previous-line)))
    ("C-k" . ("down (text)" . ,(hx :let (line-move-visual nil) :re-sel :eval next-line)))
    ...
    ("o" . ("insert below" . ,(hx :rec c :eval hx-no-sel (modaled-set-state "insert") end-of-line newline)))
    ...
    (("M-P" "M-S-p") . ("paste before from kill-ring)" . ,(hx :eval (hx-paste (read-from-kill-ring "To paste: ") -1) hx-no-sel)))
    ("M-p" . ("paste after from kill-ring)" . ,(hx :eval (hx-paste (read-from-kill-ring "To paste: ") +1) hx-no-sel)))
    ("M" . ("join lines" . hx-join-lines))
    ("u" . ("undo" . ,(hx :once :eval hx-no-sel undo-fu-only-undo)))
    ("U" . ("redo" . ,(hx :once :eval hx-no-sel undo-fu-only-redo)))
    (("M-U" "M-S-u") . ("redo all" . ,(hx :once :eval hx-no-sel undo-fu-only-redo-all)))
    ...
    (("M-C" "M-S-c") . ("remove all cursors" . hx-remove-cursors))
    ...
    (("M-S" "M-S-s") . ("remove from jump list" . hx-jump-remove))
...
(modaled-define-keys
  ...
  `(("q" . ("quit window" . quit-window))
    ...
    ("SPC p g" . ("go to project" . projectile-switch-project))
    ("SPC d" . ("dired" . (keymap)))
    ...
    ("SPC b R" . ("toggle buffer read-only" . ,(hx :eval
    ...
    ("SPC i c" . ("insert char" . insert-char))
    ...
    ;; ai
    ("SPC a" . ("ai" . (keymap)))
    ("SPC a p" . ("ai provider" . (keymap)))
    ("SPC a p s" . ("provider select" . ellama-provider-select))
    ...
    ("SPC a a i" . ("ask interactively" . ellama-chat))
...
(modaled-define-keys
  ...
  `(("M-SPC" . ("toggle vterm" . vterm-toggle))
    ...
    ;; M-S-<char> works in terminal, M-<up_char> works in GUI
    (("M-J" "M-S-j") . ("move window left" . windmove-swap-states-left))
    ("M-:" . ("move window right" . windmove-swap-states-right))
    (("M-L" "M-S-l") . ("move window up" . windmove-swap-states-up))
    (("M-K" "M-S-k") . ("move window down" . windmove-swap-states-down))
    ("C-M-j" . ("shrink window horizontally" . shrink-window-horizontally))
    ("C-M-;" . ("enlarge window horizontally" . enlarge-window-horizontally))
    ("C-M-l" . ("enlarge window vertically" . enlarge-window))
    ("C-M-k" . ("shrink window vertically" . shrink-window))
    ("J" . ("prev tab" . centaur-tabs-backward))
    (":" . ("next tab" . centaur-tabs-forward))
    ("C-S-j" . ("move tab to left" . centaur-tabs-move-current-tab-to-left))
    ("C-:" . ("move tab to right" . centaur-tabs-move-current-tab-to-right))
    ("L" . ("prev tab group" . centaur-tabs-backward-group))
    ("K" . ("next tab group" . centaur-tabs-forward-group))
    ("C-s" . ("save" . hx-save))
